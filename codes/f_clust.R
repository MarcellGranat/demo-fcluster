...
proper_cluster_df <- distance_df %>% 
  ...
  transmute(time, proper = map(data, function(x) {
    ...
    x_arranged <- x %>% 
      arrange(distance) # least different on top…
    
    out <- tibble() # collector for loop
    
    while (T) {
      out <- x_arranged %>% 
        head(1) %>% # match the least different pair ^^
        bind_rows(out)
      
      x_arranged <- x_arranged %>% 
        anti_join(out[, 1]) %>% # remove the paired ones…
        anti_join(out[, 2]) # …^
      
      if (nrow(x_arranged) == 0) break # stop sorting if all paired
    }
    out
  })) %>% 
  unnest() %>%
